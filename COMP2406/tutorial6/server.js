...
const processRequest = (request, response) => {
    ...
    else if (urlObject.pathname === "/t6.css") {
        file.readFile("t6.css", (err, data) => {
            if (err) {
                throw err;
            }
            response.writeHeader(200, {
    ...
    else if (urlObject.pathname === "/t6.js") {
        file.readFile("t6.js", (err, data) => {
            if (err) {
                throw err;
            }
            response.writeHeader(200, {
    ...
    // TODO 5: receive the request for the students.json JSON file, read contents of the file from the file system, and then send the contents of this file as a response to the client
    ...
    else if (urlObject.pathname === "/updateStudent" && request.method === "PUT") {
        let body = "";
        ...
        request.on("end", () => {
            ...
            updateStudents(updatedStudent, false);
        ...
        request.on("end", () => {
            ...
            updateStudents(newStudent, true);
...
const updateStudents = (student, isNewStudent) => {
    file.readFile("students.json", (readErr, data) => {
        ...
        if (isNewStudent) {
            studentsJSON.students.push(student);
        } else {
            studentsJSON.students.forEach((existingStudent) => {
                if (existingStudent.snum === student.snum) {
                    Object.entries(student).forEach(([key, value]) => existingStudent[`${key}`] = value);
                }
            });
        }
