...
const firstNames = students.map((student) => student.fname);
console.log(firstNames);

const deansListStudents = students.filter((student) => student.egrade >= 80).map((student) => `${student.fname} ${student.lname}`);
console.log(deansListStudents);

const meanFinalGrade = students.reduce((prevStudentGrade, currStudent) => {
	return prevStudentGrade + currStudent.agrade * 0.4 + currStudent.tgrade * 0.1 + currStudent.egrade * 0.5
}, 0) / students.length;
console.log(meanFinalGrade);